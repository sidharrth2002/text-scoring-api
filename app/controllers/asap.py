...
from app.multimodal_transformers.data.load_data import process_single_text_asap, process_single_text_bursa
...
def calculate_features_bursa(text, set_num):
...
def predict_bursa(text, set_num):
    model = models[set_num]
    features = calculate_features_bursa(text, set_num)
    data = process_single_text_bursa(text, 'asap', features, keywords=get_keywords(set_num), essay_set=set_num)
    print('SHAPE')
